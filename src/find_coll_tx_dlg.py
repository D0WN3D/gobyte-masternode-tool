...
from gobyted_intf import GobytedIndexException
...
class FindCollateralTxDlg(QDialog, ui_find_coll_tx_dlg.Ui_FindCollateralTxDlg, wnd_utils.WndUtils):
    ...
    def setupUi(self):
        try:
            ...
            self.edtAddress.setText(self.gobyte_address)
    ...
    def display_utxos(self):
        ...
        if len(self.utxos):
            ...
            self.setBaseSize(sh)
            if self.read_only:
                msg = f'<span style="color:blue">Found 1000 GoByte transaction(s):' \
                      f'</span>'
            else:
                msg = f'<span style="color:blue">Found 1000 GoByte transaction(s). Click the "Apply" button to copy' \
            ...
            self.lblMessage.setText('<span style="color:red">Found no unspent 1000 GoByte transactions  '
    ...
    def load_utxos_thread(self, ctrl):
        try:
            if not self.gobyted_intf.open():
                ...
                try:
                    self.block_count = self.gobyted_intf.getblockcount()
                    self.utxos = self.gobyted_intf.getaddressutxos([self.gobyte_address])

...
class GobytedSSH(object):
    ...
    def find_gobyted_config(self):
        """
        Try to read configuration of remote gobyte daemon. In particular we need parameters concering rpc
        configuration.
        :return: tuple (gobyted_running, gobyted_config_found, gobyted config file contents as dict)
                or error string in error occured
        """
        gobyted_running = False
        gobyted_config_found = False
        if not self.ssh:
            raise Exception('SSH session not ready')
        try:
            # find gobyted process id if running
            try:
                pids = self.remote_command('ps -C "gobyted" -o pid')
            except UnknownError:
                raise Exception('is gobyted running on the remote machine?')
            pid = None
            ...
            config = {}
            if pid:
                gobyted_running = True
                # using gobyted pid find its executable path and then .gobytecore directory and finally gobyte.conf file
                ...
                if executables and len(executables) >= 1:
                    ...
                    if len(elems) == 2:
                        ...
                        gobyted_dir = os.path.dirname(executable)
                        gobyte_conf_file = gobyted_dir + '/.gobytecore/gobyte.conf'
                        conf_lines = []
                        try:
                            conf_lines = self.remote_command('cat ' + gobyte_conf_file)
                        except Exception as e:
                            # probably error no such file or directory
                            # try to read gobyted's cwd + cmdline
                            cwd_lines = self.remote_command('ls -l /proc/' + str(pid) + '/cwd')
                            if cwd_lines:
                                elems = cwd_lines[0].split('->')
                                if len(elems) >= 2:
                                    cwd = elems[1]
                                    gobyte_conf_file = cwd + '/.gobytecore/gobyte.conf'
                                    try:
                                        conf_lines = self.remote_command('cat ' + gobyte_conf_file)
                                    except Exception as e:
                                        # second method did not suceed, so assume, that conf file is located
                                        # i /home/<username>/.gobytecore directory
                                        gobyte_conf_file = '/home/' + self.username + '/.gobytecore/gobyte.conf'
                                        conf_lines = self.remote_command('cat ' + gobyte_conf_file)
                        ...
                        gobyted_config_found = True
            return gobyted_running, gobyted_config_found, config
        except Exception as e:
            return str(e)
...
class GobytedIndexException(JSONRPCException):
    """
    Exception for notifying, that gobyte daemon should have indexing option tuned on
    """
    def __init__(self, parent_exception):
        JSONRPCException.__init__(self, parent_exception.error)
        self.message = self.message + \
                       '\n\nMake sure the gobyte daemon you are connecting to has the following options enabled in ' \
                       'its gobyte.conf:\n\n' + \
                       'addressindex=1\n' + \
                       'spentindex=1\n' + \
                       'timestampindex=1\n' + \
                       'txindex=1\n\n' + \
                       'Changing these parameters requires to execute gobyted with "-reindex" option (linux: ./gobyted -reindex)'


def control_rpc_call(func):
    """
    Decorator function for catching HTTPConnection timeout and then resetting the connection.
    :param func: GobytedInterface's method decorated
    """
            ...
            for try_nr in range(1, 5):
                try:
                    try:
                        ...
                        # exceptions raised most likely by not functioning gobyted node; try to switch to another node
                        # if there is any in the config
                        logging.error('Error while calling of "' + str(func) + ' (3)". Details: ' + str(e))
                        raise GobytedConnectionError(e)

                except GobytedConnectionError as e:
...
def json_cache_wrapper(func, intf, cache_file_ident):
    ...
    def json_call_wrapper(*args, **kwargs):
        cache_file = intf.config.cache_dir + '/insight_gobyte_' + cache_file_ident + '.json'
...
class GobytedInterface(WndUtils):
    ...
    def switch_to_next_config(self):
        """
        If there is another gobyted config not used recently, switch to it. Called only when there was a problem
    ...
    def open(self):
        """
        Opens connection to gobyte RPC. If it fails, then the next enabled conn config will be used, if any exists.
        ...
        try:
            if not self.cur_conn_def:
                raise Exception('There is no connections to GoByte network enabled in the configuration.')
            ...
            while True:
                try:
                    ...
                    return False
                        ...
                        NoValidConnectionsError) as e:
                    # exceptions raised by not likely functioning gobyted node; try to switch to another node
                    # if there is any in the config
                    if not self.switch_to_next_config():
    ...
    def open_internal(self):
        """
        Try to establish connection to gobyte RPC daemon for current connection config.
        ...
        if not self.active:
            ...
            if self.cur_conn_def.use_ssh_tunnel:
                # RPC over SSH
                if self.ssh is None:
                    self.ssh = GobytedSSH(self.cur_conn_def.ssh_conn_cfg.host, self.cur_conn_def.ssh_conn_cfg.port,
        ...
        if self.open():
            ...
            if verify_node:
                ...
                if self.config.is_testnet() and not node_under_testnet:
                    raise Exception('This RPC node works under GoByte MAINNET, but your current configuration is '
                                    'for TESTNET.')
                elif self.config.is_mainnet() and node_under_testnet:
                    raise Exception('This RPC node works under GoByte TESTNET, but your current configuration is '
                                    'for MAINNET.')
            return info
        else:
            raise Exception('Not connected')
    ...
    def issynchronized(self):
        if self.open():
            # if connecting to HTTP(S) proxy do not check if gobyte daemon is synchronized
    ...
    def update_mn_queue_values(self):
        ...
        for mn in self.masternodes:
            if mn.status == 'ENABLED'or 'WATCHDOG_EXPIRED':
    ...
    def get_masternodelist(self, *args, data_max_age=MASTERNODES_CACHE_VALID_SECONDS):
        """
        Returns masternode list, read from the GoByte network or from the internal cache.
        ...
        if self.open():
            ...
            if len(args) == 1 and args[0] == 'full':
                last_read_time = app_cache.get_value(f'MasternodesLastReadTime_{self.app_config.gobyte_network}', 0, int)
                ...
                if self.masternodes and data_max_age > 0 and \
                   int(time.time()) - last_read_time < data_max_age:
                    ...
                    logging.info('Loading masternode list from GoByte daemon...')
                    ...
                    try:
                        ...
                        app_cache.set_value(f'MasternodesLastReadTime_{self.app_config.gobyte_network}', int(time.time()))
                        self.update_mn_queue_values()
                    finally:
                        if db_modified:
                            self.db_intf.commit()

...
class SendFundsDestinationItem(QObject):
    ...
    def display_second_unit_value(self):
        if self.values_unit == OUTPUT_VALUE_UNIT_AMOUNT:
            ...
            if self.value_amount is not None:
                self.lbl_second_unit_value.setText(app_utils.to_string(round(self.value_amount, 8)) + ' GoByte')
            else:
                self.lbl_second_unit_value.setText('')
    ...
    def validate(self):
        ...
        if not address:
            valid = False
        elif not gobyte_utils.validate_address(address, self.app_config.gobyte_network):
            valid = False
        else:
            self.message = None
        if valid:
...
class SendFundsDestination(QtWidgets.QWidget, WndUtils):
    ...
    def setupUi(self, Form):
        ...
        # label dedicated to the second-unit value (e.g percentage if the main unit is set to (GoByte) amount value)
        ...
        self.lbl_fee.setText('Fee [GoByte]')
    ...
    def use_all_funds_for_address(self, address_item):
        ...
        if self.values_unit == OUTPUT_VALUE_UNIT_AMOUNT:
            ...
            # in this mode, due to the pct -> gobyte conversion for each of the outputs, there can be left a reminder,
    ...
    def calculate_the_change(self) -> float:
        """Returns the change value in GoByte."""
        sum = 0.0
        for idx, addr in enumerate(self.recipients):
    ...
    def update_the_change_ui(self):
        ...
        if self.values_unit == OUTPUT_VALUE_UNIT_AMOUNT:
            ...
            # pct
            the_change_second_unit_str = app_utils.to_string(self.change_amount) + ' GoByte'
            if self.inputs_total_amount - self.fee_amount - self.add_to_fee > 0:
    ...
    def display_totals(self):
        ...
        text = f'<span class="label"><b>Total value of selected inputs:</b>&nbsp;</span><span class="value">&nbsp;{self.inputs_total_amount} GoByte&nbsp;</span>'
        ...
        try:
            ...
            for line_idx, line in enumerate(data.split('\n')):
                if line:
                    ...
                    address_valid = gobyte_utils.validate_address(address, gobyte_network=None)

...
def connect_hw(hw_session: Optional[HwSessionInfo], hw_type: HWType, device_id: Optional[str] = 'NFC',
    ...
    def get_session_info_trezor(cli, hw_session: HwSessionInfo):
        path = gobyte_utils.get_default_bip32_base_path(hw_session.app_config.gobyte_network)
        path_n = gobyte_utils.bip32_path_string_to_n(path)
...
def get_address_ext(hw_session: HwSessionInfo,
    ...
    try:
        ...
        path_str = gobyte_utils.bip32_path_n_to_string(bip32_path_n)
        ...
        if not address:
            ...
            if row:
                ...
                try:
                    ...
                    if not gobyte_utils.validate_address(address, hw_session.app_config.gobyte_network):
                        address = None
                except Exception:
                    address = None

...
from gobyte_utils import bip32_path_n_to_string
from gobyted_intf import GobytedInterface, GobytedIndexException
...
class MainWindow(QMainWindow, WndUtils, ui_main_dlg.Ui_MainWindow):
    ...
    def __init__(self, app_path):
        ...
        self.gobyted_intf = GobytedInterface(window=None,
        ...
        self.hw_session = HwSessionInfo(
            ...
            self.config,
            gobyted_intf=self.gobyted_intf)

        self.gobyted_info = {}
        self.is_gobyted_syncing = False
        self.gobyted_connection_ok = False
        self.connecting_to_gobyted = False
    ...
    def setupUi(self):
        ...
        self.gobyted_intf.initialize(self.config)
    ...
    def closeEvent(self, event):
        ...
        if self.gobyted_intf:
            self.gobyted_intf.disconnect()
    ...
    def display_window_title(self):
        ...
        if self.config.gobyte_network == 'TESTNET':
    ...
    def check_for_updates_thread(self, ctrl, force_check):
        ...
        try:
            ...
            response = urllib.request.urlopen(
                'https://raw.githubusercontent.com/D0WN3D/gobyte-masternode-tool/master/version.txt')
    ...
    def on_action_open_settings_window_triggered(self):
        ...
        if res and dlg.get_is_modified():
            ...
            self.gobyted_intf.reload_configuration()
            if gobyte_network_sav != self.config.gobyte_network or hw_type_sav != self.config.hw_type:
                self.disconnect_hardware_wallet()
            self.display_window_title()
            self.update_edit_controls_state()
        del dlg
    ...
    def show_connection_initiated(self):
        """Shows status information related to a initiated process of connection to a gobyte RPC. """
        self.setStatus1Text('<b>RPC network status:</b> trying %s...' % self.gobyted_intf.get_active_conn_description(), 'black')
    ...
    def show_connection_failed(self):
        ...
        self.setStatus1Text('<b>RPC network status:</b> failed connection to %s' % self.gobyted_intf.get_active_conn_description(), 'red')

    def show_connection_successful(self):
        """Shows status information after successful connetion to a GoByte RPC node."""
        self.setStatus1Text('<b>RPC network status:</b> OK (%s)' % self.gobyted_intf.get_active_conn_description(), 'green')

    def show_connection_disconnected(self):
        """Shows status message related to disconnection from GoByte RPC node."""
        self.setStatus1Text('<b>RPC network status:</b> not connected', 'black')

    def checkGobytedConnection(self, wait_for_check_finish=False, call_on_check_finished=None):
        """
        Connects do gobyte daemon if not connected before and returnes if it was successful.
        ...
        def wait_for_synch_finished_thread(ctrl):
            """
            Thread waiting for gobyte daemon to finish synchronizing.
            """
            mtx = QMutex()
            cond = QWaitCondition()
            try:
                ...
                while not ctrl.finish:
                    synced = self.gobyted_intf.issynchronized()
                    if synced:
                        self.is_gobyted_syncing = False
                        self.show_connection_successful()
                        break
                    mnsync = self.gobyted_intf.mnsync()
                    self.setMessage('Gobyted is synchronizing: AssetID: %s, AssetName: %s' %
                ...
                mtx.unlock()
                self.wait_for_gobyted_synced_thread = None

        def connect_thread(ctrl):
            """
            Test connection to gobyte network inside a thread to avoid blocking GUI.
            :param ctrl: control structure to communicate with WorkerThread object (not used here)
            """
            try:
                synced = self.gobyted_intf.issynchronized()
                self.gobyted_info = self.gobyted_intf.getinfo(verify_node=True)
                self.gobyted_connection_ok = True
                if not synced:
                    logging.info("gobyted not synced")
                    if not self.is_gobyted_syncing and not (hasattr(self, 'wait_for_gobyted_synced_thread') and
                                                                  self.wait_for_gobyted_synced_thread is not None):
                        self.is_gobyted_syncing = True
                        self.wait_for_gobyted_synced_thread = self.run_thread(self, wait_for_synch_finished_thread, (),
        ...
        if self.config.is_config_complete():
            if not hasattr(self, 'check_conn_thread') or self.check_conn_thread is None:
                ...
                if hasattr(self, 'wait_for_gobyted_synced_thread') and self.wait_for_gobyted_synced_thread is not None:
                    if call_on_check_finished is not None:
                        # if a thread waiting for gobyted to finish synchronizing is running, call the callback function
                        call_on_check_finished()
                else:
                    self.connecting_to_gobyted = True
    ...
    def on_action_check_network_connection_triggered(self):
        def connection_test_finished():
            ...
            if self.gobyted_connection_ok:
                if self.is_gobyted_syncing:
                    self.infoMsg('Connection successful, but GoByte daemon is synchronizing.')
                else:
                    self.infoMsg('Connection successful.')
            else:
                if self.gobyted_intf.last_error_message:
                    self.errorMsg('Connection error: ' + self.gobyted_intf.last_error_message)
                else:
        ...
        if self.config.is_config_complete():
            ...
            # disable all actions that utilize gobyte network
            ...
            self.checkGobytedConnection(call_on_check_finished=connection_test_finished)
        else:
        ...
        if not self.hw_client:
            try:
                try:
                    ...
                    if self.config.gobyte_network == 'TESTNET':
                        # check if GoByte testnet is supported by this hardware wallet
                        ...
                        if self.config.hw_type in (HWType.trezor, HWType.keepkey):
                            ...
                            addr = hw_intf.get_address(self.hw_session,
                                                       gobyte_utils.get_default_bip32_path(self.config.gobyte_network))
                            if gobyte_utils.validate_address(addr, self.config.gobyte_network):
                                found_testnet_support = False

                        if not found_testnet_support:
                            url = get_note_url('GMT0002')
                            msg = f'Your hardware wallet device does not support GoByte TESTNET ' \
                        ...
                        self.config.hw_coin_name = 'GoByte'
    ...
    def scan_hw_for_bip32_paths(self, addresses) -> Tuple[Dict[str, str], bool]:
        """
        Scans hardware wallet for bip32 paths of all GoByte addresses passed in the addresses list.
        :param addresses: list of GoByte addresses to scan
        :return: Tuple[Dict[str <gobyte address>, str <bip32 path>], bool <True if user cancelled scanning>]
        """
        paths_found = []
        ...
        def scan_for_bip32_thread(ctrl, addresses):
            ...
            for a bip32 paths with given GoByte addresses.
            :param cfg: Thread dialog configuration object.
            :param addresses: list of GoByte addresses to find bip32 path
            :return: 
            """
            ...
            if self.hw_client:
                ...
                address_n = gobyte_utils.get_default_bip32_base_path_n(self.config.gobyte_network) + [None, 0, None]
                db_cur = self.config.db_intf.get_cursor()

                try:
                    for addr_to_find_bip32 in addresses:
                        if not found_adresses.get(addr_to_find_bip32):
                            # check 10 addresses of account 0 (44'/176'/0'/0), then 10 addreses
                            # of account 1 (44'/176'/1'/0) and so on until 9th account.
                            # if not found, then check next 10 addresses of account 0 (44'/176'/0'/0)
                    ...
                    with open(file_name, 'r') as f_ptr:
                        ...
                        for line in f_ptr.readlines():
                            ...
                            if len(elems) >= 5 and not line.startswith('#'):
                                ...
                                def update_mn(in_mn):
                                    ...
                                    in_mn.collateralAddress = mn_gobyte_addr
                        ...
                        if imported_cnt:
                            ...
                            if self.queryDlg(message=msg_text, buttons=QMessageBox.Yes | QMessageBox.No,
                                ...
                                # scan all GoByte addresses from imported masternodes for BIP32 path, starting from
                                # first standard GoByte BIP32 path
    ...
    def generate_mn_priv_key(self, compressed: bool):
        ...
        wif = gobyte_utils.generate_privkey(self.config.gobyte_network, compressed=compressed)
    ...
    def on_btnHwBip32ToAddress_clicked(self):
        """
        Convert BIP32 path to GoByte address.
        :return: 
        """
        try:
            self.connect_hardware_wallet()
            ...
            if self.curMasternode and self.curMasternode.collateralBip32Path:
                gobyte_addr = hw_intf.get_address(self.hw_session, self.curMasternode.collateralBip32Path)
                self.edtMnCollateralAddress.setText(gobyte_addr)
                self.curMasternode.collateralAddress = gobyte_addr
                self.curMnModified()
        except HardwareWalletCancelException:
            if self.hw_client:
                self.hw_client.init_device()
    ...
    def on_btnHwAddressToBip32_clicked(self):
        """
        Converts GoByte address to BIP32 path, using hardware wallet.
        :return: 
        """
            ...
            if self.curMasternode and self.curMasternode.collateralAddress:
                ...
                if not user_cancelled:
                    if not paths or len(paths) == 0:
                        self.errorMsg("Couldn't find GoByte address in your hardware wallet. If you are using HW passphrase, "
    ...
    def on_btnBroadcastMn_clicked(self):
        ...
        self.checkGobytedConnection(wait_for_check_finish=True)
        if not self.gobyted_connection_ok:
            self.errorMsg("Connection to GoByte daemon is not established.")
            return
        if self.is_gobyted_syncing:
            self.warnMsg("GoByte daemon to which you are connected is synchronizing. You have to wait "
                         "until it's finished.")
            return
        ...
        try:
            mn_privkey = gobyte_utils.wif_to_privkey(self.curMasternode.privateKey, self.config.gobyte_network)
            if not mn_privkey:
            ...
            seq = 0xffffffff
            block_count = self.gobyted_intf.getblockcount()
            block_hash = self.gobyted_intf.getblockhash(block_count - 12)
            ...
            if not cfg_collateral_address:
                ...
                if self.queryDlg(message="The GoByte address retrieved from the hardware wallet (%s) for the configured "
            ...
            try:
                ...
                if found:
                    if utxo.get('satoshis', None) != 100000000000:
                        if self.queryDlg(
                                message="Collateral transaction output should equal 100000000000 Satoshis (1000 GoByte)"
            ...
            collateral_in = gobyte_utils.num_to_varint(len(collateral_pubkey)).hex() + collateral_pubkey.hex()
            delegate_in = gobyte_utils.num_to_varint(len(mn_pubkey) / 2).hex() + mn_pubkey
            sig_time = int(time.time())

            info = self.gobyted_intf.getinfo(verify_node=True)
            ...
            last_ping_serialize_for_sig = gobyte_utils.serialize_input_str(
                self.curMasternode.collateralTx,
                self.curMasternode.collateralTxIndex,
                seq,
                '') + block_hash + str(sig_time)

            r = gobyte_utils.ecdsa_sign(last_ping_serialize_for_sig, self.curMasternode.privateKey,
                                      self.config.gobyte_network)
            ...
            work = vintx + vinno + vinsig + vinseq \
                   ...
                   + gobyte_utils.num_to_varint(len(sig1) / 2).hex() + sig1 \
            ...
            if ret['overall'].startswith('Successfully decoded broadcast messages for 1 masternodes'):
                ...
                ret = self.gobyted_intf.masternodebroadcast("relay", work)
                ...
                if failed_count == 0:
                    ...
                    self.errorMsg('Failed to start masternode.\n\nResponse from GoByte daemon: %s.' % errorMessage)
            else:
    ...
    def get_masternode_status(self, masternode):
        ...
        if self.gobyted_connection_ok:
            ...
            mns_info = self.gobyted_intf.get_masternodelist('full', collateral_id)
            if len(mns_info):
    ...
    def get_masternode_status_description(self):
        ...
        if self.gobyted_connection_ok:
            ...
            mns_info = self.gobyted_intf.get_masternodelist('full', data_max_age=120)  # read new data from the network
                                                                                     # every 120 seconds
            mn_info = self.gobyted_intf.masternodes_by_ident.get(collateral_id)
            if mn_info:
                ...
                if mn_info.status == 'ENABLED' or mn_info.status == 'PRE_ENABLED':
                    ...
                    color = 'red'
                enabled_mns_count = len(self.gobyted_intf.payment_queue)
                ...
                if addr:
                    try:
                        bal = self.gobyted_intf.getaddressbalance([addr])
                        if bal:
            ...
            status = '<span style="color:red">Problem with connection to gobyted.</span>'
        return status
    ...
    def on_btnRefreshMnStatus_clicked(self):
        ...
        self.checkGobytedConnection(wait_for_check_finish=True, call_on_check_finished=enable_buttons)
        if self.gobyted_connection_ok:
            try:
    ...
    def on_action_transfer_funds_for_cur_mn_triggered(self):
        ...
        if self.curMasternode:
            ...
            if not self.curMasternode.collateralBip32Path:
                ...
                self.errorMsg("Enter the masternode collateral GoByte address. You can use the 'left arrow' "
    ...
    def on_btnFindCollateral_clicked(self):
        """
        Open dialog with list of utxos of collateral gobyte address.
        :return: 
        """
        if self.curMasternode and self.curMasternode.collateralAddress:
            ui = FindCollateralTxDlg(self, self.gobyted_intf, self.curMasternode.collateralAddress,
    ...
    def on_action_open_proposals_window_triggered(self):
        ui = ProposalsDlg(self, self.gobyted_intf)
        ui.exec_()

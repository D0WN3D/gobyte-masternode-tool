...
class PaymentTableModel(QAbstractTableModel):
    def __init__(self, parent, hide_collaterals_utxos, parent_wnd):
        ...
        self.columns = [
            ...
            ('satoshis', 'Amount (GoByte)', True, 100),
...
UtxoSrcAddr = Tuple[str, str]  # GoByte address (in HW) to be scanned for UTXOs (address, bip32 path)
...
class WalletDlg(QDialog, ui_send_payout_dlg.Ui_SendPayoutDlg, WndUtils):
    ...
    def on_lbl_hw_account_base_path_linkActivated(self, text):
        ...
        if ok:
            try:
                gobyte_utils.bip32_path_string_to_n(path)
        ...
        def apply_utxos(new_utxos: List[dict], existing_utxos: List[dict]):
            ...
            if self.utxo_src_mode == 1:
                if self.mn_src_index is not None and self.mn_src_index < len(self.masternode_addresses) and \
                        self.mn_src_index >= 0:
                    msg = f'<b>Balance:</b> {self.sel_addresses_balance} GoByte&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} GoByte&nbsp;&nbsp;&nbsp;<b>Address:</b> {self.masternode_addresses[self.mn_src_index][0]}'
                    self.set_message_2(msg)
                else:
                    msg = f'<b>Balance:</b> {self.sel_addresses_balance} GoByte&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} GoByte'
                    self.set_message_2(msg)
            elif self.utxo_src_mode == 2:
                msg = f'<b>Balance:</b> {self.sel_addresses_balance} GoByte&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} GoByte'
                self.set_message_2(msg)
            elif self.utxo_src_mode in (1, 3):
                msg = f'<b>Balance:</b> {self.sel_addresses_balance} GoByte&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} GoByte&nbsp;&nbsp;&nbsp;<b>Address:</b> {self.hw_src_address}'
                self.set_message_2(msg)

    ...
    def fetch_json(self, url, resource, resourceid):
        ...
        if self.cache_dir:
            ...
            try: # looking into cache first
                ...
                pass
        try:
            j = self.gobyted_inf.getrawtransaction(resourceid, 1)
        except Exception as e:
            raise
        if cache_file:
...
def prepare_transfer_tx(hw_session: HwSessionInfo, utxos_to_spend: List[dict], dest_addresses: List[Tuple[str, int, str]], tx_fee):
    ...
    :param dest_address: destination (GoByte) address
    :param tx_fee: transaction fee
    :return: tuple (serialized tx, total transaction amount in satoshis)
    """

    insight_network = 'insight_gobyte'
    if hw_session.app_config.is_testnet():
        insight_network += '_testnet'
    gobyte_network = hw_session.app_config.gobyte_network

    tx_api = MyTxApiInsight(insight_network, '', hw_session.gobyted_intf, hw_session.app_config.cache_dir)
    ...
    for addr, amount, bip32_path in dest_addresses:
        ...
        if addr[0] in gobyte_utils.get_chain_params(gobyte_network).B58_PREFIXES_SCRIPT_ADDRESS:
            stype = proto_types.PAYTOSCRIPTHASH
            logging.debug('Transaction type: PAYTOSCRIPTHASH' + str(stype))
        elif addr[0] in gobyte_utils.get_chain_params(gobyte_network).B58_PREFIXES_PUBKEY_ADDRESS:

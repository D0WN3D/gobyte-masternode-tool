...
class ChainParamsTestNet(ChainParams):
    B58_PREFIXES_PUBKEY_ADDRESS = ['n']
    B58_PREFIXES_SCRIPT_ADDRESS = ['9']
    B58_PREFIXES_SECRET_KEY = ['9', 'c']
    PREFIX_PUBKEY_ADDRESS = 112
    PREFIX_SCRIPT_ADDRESS = 20
    PREFIX_SECRET_KEY = 240
    BIP44_COIN_TYPE = 1


def get_chain_params(gobyte_network: str) -> typing.ClassVar[ChainParams]:
    if gobyte_network == 'MAINNET':
        return ChainParamsMainNet
    elif gobyte_network == 'TESTNET':
        return ChainParamsTestNet
    else:
        raise Exception('Invalid \'network\' value.')


def get_default_bip32_path(gobyte_network: str):
    return bip32_path_n_to_string([44 + 0x80000000, get_chain_params(gobyte_network).BIP44_COIN_TYPE + 0x80000000, 0x80000000, 0, 0])


def get_default_bip32_base_path(gobyte_network: str):
    return bip32_path_n_to_string([44 + 0x80000000, get_chain_params(gobyte_network).BIP44_COIN_TYPE + 0x80000000])


def get_default_bip32_base_path_n(gobyte_network: str):
    return [44 + 0x80000000, get_chain_params(gobyte_network).BIP44_COIN_TYPE + 0x80000000]
...
def pubkey_to_address(pub_key, gobyte_network: str):
    """Convert public key to GoByte address."""
    pubkey_bin = bytes.fromhex(pub_key)
    pub_hash = bitcoin.bin_hash160(pubkey_bin)
    data = bytes([get_chain_params(gobyte_network).PREFIX_PUBKEY_ADDRESS]) + pub_hash
    checksum = bitcoin.bin_dbl_sha256(data)[0:4]
    return base58.b58encode(data + checksum)


def validate_address(address: str, gobyte_network: typing.Optional[str]) -> bool:
    """Validates if the 'address' is a valid GoByte address.
    :address: address to be validated
    :gobyte_network: the gobyte network type against which the address will be validated; if the value is None, then
    ...
    if len(data) > 5:
        prefix = data[0]
        if gobyte_network:
            prefix_valid = (prefix == get_chain_params(gobyte_network).PREFIX_PUBKEY_ADDRESS)
        else:
...
def generate_privkey(gobyte_network: str, compressed: bool = False):
    """
    ...
    if compressed:
        privkey += '01'
    data = bytes([get_chain_params(gobyte_network).PREFIX_SECRET_KEY]) + bytes.fromhex(privkey)
    checksum = bitcoin.bin_dbl_sha256(data)[0:4]
    return base58.b58encode(data + checksum)
...
def num_to_varint(a):
    """
    Based on project: https://github.com/chaeplin/gobytemnb
    """
    x = int(a)
    if x < 253:
...
def wif_to_privkey(wif_key: str, gobyte_network: str):
    """
    Based on project: https://github.com/chaeplin/gobytemnb with some changes related to usage of bitcoin library.
    """
    wif_compressed = (52 == len(wif_key))
    privkey_encoded = base58.b58decode(wif_key).hex()
    wif_version = privkey_encoded[:2]
    wif_prefix = get_chain_params(gobyte_network).PREFIX_SECRET_KEY
...
def ecdsa_sign(msg: str, wif_priv_key: str, gobyte_network: str):
    """Signs a message with the Elliptic Curve algorithm.
    Note: GoByte core uses uncompressed public keys, so if the private key passed as an argument
    ...
    pubkey = bitcoin.privkey_to_pubkey(wif_to_privkey(wif_priv_key, gobyte_network))
...
def compose_tx_locking_script(dest_address, gobyte_newtork: str):
    """
    ...
    if dest_address[0] in get_chain_params(gobyte_newtork).B58_PREFIXES_PUBKEY_ADDRESS:
        ...
        scr = OP_DUP + \
              ...
              OP_EQUALVERIFY + \
              OP_CHECKSIG
    elif dest_address[0] in get_chain_params(gobyte_newtork).B58_PREFIXES_SCRIPT_ADDRESS:

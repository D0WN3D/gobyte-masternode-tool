...
class AppConfig(object):
    def __init__(self):
        ...
        # List of GoByte network configurations. Multiple conn configs advantage is to give the possibility to use
        # another config if particular one is not functioning (when using "public" RPC service, it could be node's
        # maintanance)
        self.gobyte_net_configs = []

        # to distribute the load evenly over "public" RPC services, we choose radom connection (from enabled ones)
        # if it is set to False, connections will be used accoording to its order in gobyte_net_configs list
        self.random_gobyte_net_config = True

        # list of all enabled gobyted configurations (GoByteNetworkConnectionCfg) - they will be used accourding to
        # the order in list
        self.active_gobyte_net_configs = []

        # list of misbehaving gobyte network configurations - they will have the lowest priority during next
        # connections
        ...
        self.gobyte_network = 'MAINNET'

        self.block_explorer_tx_mainnet = 'https://insight.gobyte.network/insight/tx/%TXID%'
        self.block_explorer_addr_mainnet = 'https://insight.gobyte.network/insight/address/%ADDRESS%'
        self.block_explorer_tx_testnet = 'https://test.insight.gobyte.siampm.com/tx/%TXID%'
        self.block_explorer_addr_testnet = 'https://test.insight.gobyte.siampm.com/address/%ADDRESS%'
        self.gobyte_central_proposal_api = 'https://masternodes.gobyte.network/proposaldetails.html?proposalhash=%HASH%'
        ...
        # runtime information, set after connecting to hardware wallet device; for GoByte mainnet the value is
        # 'GoByte', for GoByte testnet, the value is 'GoByte Testnet' or 'tGoByte' depending on the (custom) firmware
        # used
        self.hw_coin_name = 'GoByte'
    ...
    def init(self, app_path):
        ...
        cache_file_name = os.path.join(self.cache_dir, 'gmt_cache.json')
    ...
    def copy_from(self, src_config):
        self.gobyte_network = src_config.gobyte_network
        self.gobyte_net_configs = copy.deepcopy(src_config.gobyte_net_configs)
        self.random_gobyte_net_config = src_config.random_gobyte_net_config
        ...
        self.gobyte_central_proposal_api = src_config.gobyte_central_proposal_api
    ...
    def configure_cache(self):
        if self.is_testnet():
            db_cache_file_name = 'gmt_cache_testnet.db'
        else:
            db_cache_file_name = 'gmt_cache.db'
    ...
    def clear_configuration(self):
        ...
        :return:
        """
        self.gobyte_net_configs.clear()
        self.active_gobyte_net_configs.clear()
    ...
    def read_from_file(self, hw_session: HwSessionInfo, file_name: Optional[str] = None,
        ...
        if os.path.exists(file_name):
            ...
            try:
                ...
                gobyte_network = config.get(section, 'gobyte_network', fallback='MAINNET')
                if gobyte_network not in ('MAINNET', 'TESTNET'):
                    logging.warning(f'Invalid gobyte_network value: {gobyte_network}')
                    gobyte_network = 'MAINNET'
                self.gobyte_network = gobyte_network

                if self.is_mainnet():
                    def_bip32_path = "44'/176'/0'/0/0"
                else:
                ...
                self.random_gobyte_net_config = self.value_to_bool(config.get(section, 'random_gobyte_net_config',
                                                                            fallback='1'))
                ...
                for section in config.sections():
                    if re.match('MN\d', section):
                        ...
                        cfg = GoByteNetworkConnectionCfg('rpc')
                        ...
                        self.gobyte_net_configs.append(cfg)
                        if correct_public_nodes:
                            if cfg.host.lower() == 'gmt1.gobyte-gmt.eu':
                                cfg.host = 'gmt1.gobytenation.org'
                                cfg.port = '443'
                                configuration_corrected = True
                            elif cfg.host.lower() == 'gmt2.gobyte-gmt.eu':
                                cfg.host = 'gmt2.gobytenation.org'
                                cfg.port = '443'
                                configuration_corrected = True
        ...
        try:
            cfgs = self.decode_connections(default_config.gobyted_default_connections)
            if cfgs:
    ...
    def save_to_file(self, hw_session: HwSessionInfo, file_name: Optional[str] = None):
        ...
        config.set(section, 'gobyte_network', self.gobyte_network)
        if not self.hw_type:
            self.hw_type = HWType.trezor
        ...
        config.set(section, 'random_gobyte_net_config', '1' if self.random_gobyte_net_config else '0')
        ...
        # save gobyte network connections
        for idx, cfg in enumerate(self.gobyte_net_configs):
    ...
    def is_config_complete(self):
        for cfg in self.gobyte_net_configs:
            if cfg.enabled:
                return True
        return False
    ...
    def prepare_conn_list(self):
        """
        Prepare list of enabled connections for connecting to gobyte network. 
        :return: list of GoByteNetworkConnectionCfg objects order randomly (random_gobyte_net_config == True) or according 
        ...
        for cfg in self.gobyte_net_configs:
            if cfg.enabled and self.is_testnet() == cfg.testnet:
                tmp_list.append(cfg)
        if self.random_gobyte_net_config:
            ordered_list = []
            while len(tmp_list):
                idx = randint(0, len(tmp_list)-1)
                ordered_list.append(tmp_list[idx])
                del tmp_list[idx]
            self.active_gobyte_net_configs = ordered_list
        else:
            self.active_gobyte_net_configs = tmp_list

    def get_ordered_conn_list(self):
        if not self.active_gobyte_net_configs:
            self.prepare_conn_list()
        return self.active_gobyte_net_configs
    ...
    def decode_connections(self, raw_conn_list):
        """
        Decodes list of dicts describing connection to a list of GoByteNetworkConnectionCfg objects.
        :param raw_conn_list: 
    ...
    def decode_connections_json(self, conns_json):
        ...
        :return: list of GoByteNetworkConnectionCfg objects or None if there was an error while importing
        """
        try:
    ...
    def import_connections(self, in_conns, force_import, limit_to_network: Optional[str]):
        ...
        :param in_conns: list of GoByteNetworkConnectionCfg objects.
        ...
        if in_conns:
            ...
            for conn in self.gobyte_net_configs:
                if conn.testnet:
            ...
            for nc in in_conns:
                if (self.gobyte_network == 'MAINNET' and nc.testnet == False) or \
                   (self.gobyte_network == 'TESTNET' and nc.testnet == True) or not limit_to_network:
                    id = nc.get_conn_id()
                    ...
                    if not conn:
                        if force_import or not app_cache.get_value('imported_default_conn_' + nc.get_conn_id(),
                           ...
                           (testnet_conn_count == 0 and nc.testnet) or  (mainnet_conn_count == 0 and nc.mainnet):
                            ...
                            self.gobyte_net_configs.append(nc)
    ...
    def get_conn_cfg_by_id(self, id):
        """
        Returns GoByteNetworkConnectionCfg object by its identifier or None if does not exists.
        :param id: Identifier of the sought connection.
        :return: GoByteNetworkConnectionCfg object or None if does not exists.
        """
        for conn in self.gobyte_net_configs:
    ...
    def is_testnet(self) -> bool:
        return self.gobyte_network == 'TESTNET'

    def is_mainnet(self) -> bool:
        return self.gobyte_network == 'MAINNET'

    def get_block_explorer_tx(self):
        if self.gobyte_network == 'MAINNET':
...
class GoByteNetworkConnectionCfg(object):
    ...
    def identical(self, cfg2):
        ...
        :param cfg2: GoByteNetworkConnectionCfg object to compare
    ...
    def is_http_proxy(self):
        ...
        proxy uses SSL while normal, "local" gobyted does not. 
        """
        if self.__use_ssl:
            return True
        else:

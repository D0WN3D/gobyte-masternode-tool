...
from gobyted_intf import GobytedIndexException, Masternode
...
# will display data from cache, instead of requesting them from a gobyte daemon, which is
# more time consuming.
...
class VotingMasternode(AttrsProtected):
    def __init__(self, masternode, masternode_config):
        ...
        :param masternode: ref to an object storing mn information read from the network (gobyted_intf.Masternode)
...
class Proposal(AttrsProtected):
    ...
    def apply_values(self, masternodes, last_superblock_time, next_superblock_datetime):
        ...
        cycle_blocks = gi.get('superblockcycle', 17280)
        ...
        if not self.get_value('title'):
            # if title value is not set (it's an external attribute, from gobytecentral) then copy value from the
            # name column
...
class ProposalsDlg(QDialog, ui_proposals.Ui_ProposalsDlg, wnd_utils.WndUtils):
    def __init__(self, parent, gobyted_intf):
    ...
    def restore_cache_settings(self):
        ...
        for idx, mn in enumerate(self.main_wnd.config.masternodes):
            ...
            if mn_ident:
                if gobyte_utils.privkey_valid(mn.privateKey):
    ...
    def display_budget_summary(self):
        ...
        if self.next_budget_approved is not None:
            budget_approved = f'<td><b>Budget approved:</b> {app_utils.to_string(round(self.next_budget_approved))} GoByte '
        ...
        if self.next_budget_requested is not None:
            budget_requested = f'<td><b>Budget requested:</b> {app_utils.to_string(round(self.next_budget_requested))} GoByte '
        ...
        #             f'{app_utils.to_string(round(self.next_budget_approved_by_user_yes_votes))} GoByte '
        #         if self.next_budget_amount:
        #             budget_approved_user_yes += \
        ...
        message = '<html><head></head><style>td{padding-right:10px}</style><body>' \
                  ...
                  f'{bra}{budget_approved_user_yes}<tr><td><b>Budget available:</b> {app_utils.to_string(round(self.next_budget_amount))} GoByte</td><td></td></tr>' \
                  f'</table></body></html>'
    ...
    def read_proposals_from_network(self):
        """ Reads proposals from the GoByte network. """
        ...
        try:
            ...
            logging.info('Reading proposals from the GoByte network.')
            begin_time = time.time()
            proposals_new = self.gobyted_intf.gobject("list", "valid", "proposals")
            ...
            if len(proposals_new) > 0:
                if errors < len(proposals_new)/10:
                    try:
                        ...
                        for prop in self.proposals:
                            ...
                            if prop.marker:
                                if not prop.db_id:
                                    ...
                                    # gobyted sometimes does not return some proposals, so they are deactivated id the db
                        ...
                        # delete proposals which no longer exists in tha GoByte network
                        rows_removed = False
                ...
                logging.warning('No proposals returned from gobyted.')
            ...
            logging.exception('Exception wile reading proposals from GoByte network.')
            self.display_message('')
            self.errorMsg('Error while reading proposals data from the GoByte network: ' + str(e))
            raise
    ...
    def read_governance_data(self):
        try:
            ...
            self.governanceinfo = self.gobyted_intf.getgovernanceinfo()
            cycle_blocks = self.governanceinfo.get('superblockcycle', 17280)
            ...
            self.next_budget_amount = float(self.gobyted_intf.getsuperblockbudget(sb_next))

            # superblocks occur every 17280 blocks (approximately 30.0 days)
            cur_block = self.gobyted_intf.getblockcount()

            sb_last_hash = self.gobyted_intf.getblockhash(sb_last)
            last_bh = self.gobyted_intf.getblockheader(sb_last_hash)
            self.last_superblock_time = last_bh['time']
            self.next_superblock_time = 0
            if cur_block > 0 and cur_block <= sb_next:
                cur_hash = self.gobyted_intf.getblockhash(cur_block)
                cur_bh = self.gobyted_intf.getblockheader(cur_hash)
            ...
            self.errorMsg("Coundn't read governanceinfo from the GoByte network. "
    ...
    def read_data_thread(self, ctrl):
        ...
        try:
            self.display_message('Connecting to GoByte daemon, please wait...')
            if not self.gobyted_intf.open():
                ...
                try:
                    ...
                    mns = self.gobyted_intf.get_masternodelist('full')
            ...
            if not self.finishing:
                # read additional data from external sources, if configured (GoByteCentral)
                proposals = []
    ...
    def read_external_attibutes(self, proposals):
        """Method reads additional proposal attributes from an external source such as GoByteCentral.org
        ...
        try:
            url = self.main_wnd.config.gobyte_central_proposal_api
            if url:
    ...
    def read_voting_from_network(self, force_reload_all, proposals):
        """
        Retrieve from a GoByte daemon voting results for all defined masternodes, for all visible Proposals.
                ...
                if not self.gobyted_intf.open():
                    ...
                    try:
                        ...
                        for row_idx, prop in enumerate(proposals):
                            ...
                            tm_begin = time.time()
                            votes = self.gobyted_intf.gobject("getvotes", prop.get_value('hash'))
    ...
    def refresh_proposals_thread(self, ctrl):
        ...
        proposals = []
        if self.main_wnd.config.read_proposals_external_attributes:
            # select proposals for which we read additional data from external sources as GoByteCentral.org
            for prop in self.proposals:
                if not prop.ext_attributes_loaded:
                    proposals.append(prop)
        if proposals and not self.finishing:
    ...
    def refresh_details_tabs(self):
        try:
            ...
            if self.current_proposal:
                ...
                if not re.match('.*gobytecentral\.org', url.lower()):
                    dc_url = 'https://www.central.gobytenation.org/p/' + prop.get_value('name')
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding" style="white-space:nowrap"><span>{app_utils.to_string(prop.get_value('payment_amount'))} GoByte&#47;cycle ({cycles_str}, {app_utils.to_string(prop.get_value('payment_amount_total'))} GoByte total)
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding">{app_utils.to_string(total_amount_requested)} GoByte{total_pct_requested}</td>
        </tr>
        <tr class="main-row">
            <td class="first-col-label">Approved from the next budget:</td>
            <td class="padding">{app_utils.to_string(total_amount_approved)} GoByte{total_pct_approved}</td>
        </tr>
    </tbody>
</table>
 </body>
        ...
        for prop in proposal_list:
            ...
            for vote_idx, mn_info in enumerate(masternodes):
                ...
                try:
                    ...
                    step = 2
                    vote_sig = gobyte_utils.ecdsa_sign(serialize_for_sig, mn_info.masternode_config.privateKey,
                                                     self.app_config.gobyte_network)

                    step = 3
                    v_res = self.gobyted_intf.voteraw(
                    ...
                    if step in (1, 4):
                        ...
                        logging.info('masternode_pub_key: %s' %
                                     str(gobyte_utils.privkey_to_pubkey(mn_info.masternode_config.privateKey)))
                        logging.info('masternode_pub_key_hash: %s' %
                                     str(gobyte_utils.pubkey_to_address(gobyte_utils.privkey_to_pubkey(
                                         mn_info.masternode_config.privateKey), self.app_config.gobyte_network)))

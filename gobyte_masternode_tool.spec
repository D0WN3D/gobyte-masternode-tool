...
a = Analysis(['src/gobyte_masternode_tool.py'],
             pathex=[base_dir],
             binaries=[],
             datas=add_files,
             hiddenimports=[],
...
if os_type == 'win32':
    ...
    os.system('"7z.exe" a %s %s -mx0' % (os.path.join(all_bin_dir, 'GoByteMasternodeTool_' + version_str + '.win' + no_bits + '.zip'),  'GoByteMasternodeTool.exe'))
elif os_type == 'darwin':
    print('Compressing Mac executable')
    os.system('zip -r "%s" "%s"' % (os.path.join(all_bin_dir, 'GoByteMasternodeTool_' + version_str + '.mac.zip'),  'GoByteMasternodeTool.app'))
elif os_type == 'linux':
    print('Compressing Linux executable')
    os.system('tar -zcvf %s %s' % (os.path.join(all_bin_dir, 'GoByteMasternodeTool_' + version_str + '.linux.tar.gz'),  'GoByteMasternodeTool'))
